...
function initooo() {
  Promise.all([client.data.get("loggedInUser"), client.iparams.get(), getIparamDB()])
    .then(function(response) {
      ...
      // window.domain = window.iparams.domain;
      // window.api_key = window.iparams.api_key;
      // window.api_key = 'wM03uZdOceEV00Vfy9VB';
...
function setupooo(data) {
  ...
  initfroala();
  getooo();
  $("#froala-editor").froalaEditor("html.set", data.emailbody);
  if (!data.canagenteditemail) {
    toggleEditor("edit.off");
    $(".fr-toolbar.fr-disabled").hide();
    $("#askadminforeditaccess").show();
  }
}
  ...
  client.db
    ...
    .then(function(data) {
      ...
      leaves.forEach(function(leave, index) {
        ...
        <span class="dateofleave">${from} ${
          from === to ? "" : " - " + to
        }</span>
        <span class="bywhom">${whom}</span>
        <div class="pad10">
          <span class="leavedays">${leaves} ${
          leaves > 1 ? "days" : "day"
        }</span>
        </div>
        </div>`;
...
function getdata(passeddate) {
  ...
  if (window.groupdata) {
    ...
    client.request.invokeTemplate("getRequestApiTemplate",{ context: { path: '/api/v2/ticket_fields?type=default_group' } }).then(function(groupdata) {
      window.groupdata = JSON.parse(groupdata.response).ticket_fields[0].choices;
      getselecteddate(date);
    })
    .catch(function(error) {
...
function getselecteddate(date) {
  var groupinfo = _.invert(window.groupdata);
  client.db
    .get(date)
    .then(function(data) {
      $(".ooolisttable ")
        ...
        data.agents.forEach(agent => {
          ...
          if (agent.groupid) {
            agent.groupid.forEach(function(group) {
              groupnames.push(groupinfo[group]);
            });
...
function saveagentinfodb(start, end, agentid) {
  ...
  var obj = {
    ...
    todate: localend
      ...
      .setZone("utc")
      .toISO(),
    email: $("#froala-editor").froalaEditor("html.get")
  };
...
function toggleEditor(method) {
  $("#froala-editor").froalaEditor(method);
}
  ...
  $(".searchinput")
    .autocomplete({
      ...
      source: function(request, response) {
        ...
        var req_path = `/search/autocomplete/itil_agents.json?term=${request.term}`
        client.request.invokeTemplate("getRequestApiTemplate", { context: { path: req_path } }).then(function(data) {
          $("#searchicon")
